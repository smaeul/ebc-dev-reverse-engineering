#include "pvi_waveform_v8_v2.11.h"
...
LAB_00100770:
    ...
    if (temperture < (int)(uint)*(byte *)&global_waveform[1].checksum) {
      temp_index = 0xffffffff;
    }
    else {
      frame_num = 0;
      do {
        temp_index = frame_num;
        if ((uint)bVar1 == temp_index + 1) goto LAB_00100770;
        frame_num = temp_index + 1;
      } while ((int)(uint)global_waveform->xwia[(long)(int)(temp_index + 0x31) + -0x1d] <=
               temperture);
    }
  }
  ...
  if (lut_type == WF_TYPE_AUTO) {
    ...
    if (frame_num == 0) goto LAB_001008d8;
  }
  else {
    mode_index = 0;
    if (frame_num == 0) {
  ...
  do {
    ...
    do {
      ...
      do {
        ...
        wf_table[out_index] =
             (byte)((wf_table[(int)(frame_start + (row_index & 0xf0) * 0x10 + (column >> 4))] & 3)
                   << 2) |
  ...
  } while (frame_count < (int)frame_num);
  if (lut_type != WF_TYPE_AUTO) {
    return 0;
  }
LAB_001008d8:
  output->frame_num = output->frame_num | mode_index << 8;
  return 0;
}
...

{
  ...
  if (bVar1 < 0x1a) {
    if ((bVar1 < 0x18) && (((bVar1 != 0x12 && (bVar1 != 0x16)) && (bVar1 != 9)))) goto LAB_0010096c;
  }
  else {
    if (bVar1 != 0x23) {
      if (bVar1 < 0x24) {
        if (bVar1 != 0x20) goto LAB_0010096c;
      }
      else {
